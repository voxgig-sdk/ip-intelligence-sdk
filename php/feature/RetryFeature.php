...
// IpIntelligence SDK retry feature
...
class IpIntelligenceRetryFeature extends IpIntelligenceBaseFeature
{
    ...
    public function init(IpIntelligenceContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (IpIntelligenceContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(IpIntelligenceContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(IpIntelligenceContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
