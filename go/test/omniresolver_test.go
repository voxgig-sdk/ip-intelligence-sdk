...
import (
	...
	omni "github.com/voxgig-sdk/ip-intelligence-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.IpIntelligenceError); is {
				out := map[string]any{
					"name":    "IpIntelligenceError",
...
func omniCtx(arg any, client *sdk.IpIntelligenceSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.IpIntelligenceSDK); is {
			client = live
