# IpIntelligence SDK test runner

from __future__ import annotations
import os
import json

from ipintelligence_sdk.utility.voxgig_struct import voxgig_struct as vs


class IpIntelligenceTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            IpIntelligenceTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = IpIntelligenceTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = IpIntelligenceTestRunner.getenv("IP_INTELLIGENCE_TEST_LIVE")
        override = IpIntelligenceTestRunner.getenv("IP_INTELLIGENCE_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = IpIntelligenceTestRunner.getenv(key)
        ...
        explain = IpIntelligenceTestRunner.getenv("IP_INTELLIGENCE_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if IpIntelligenceTestRunner._test_control is not None:
            return IpIntelligenceTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                IpIntelligenceTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            IpIntelligenceTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = IpIntelligenceTestRunner.load_test_control()
...
def load_env_local():
    IpIntelligenceTestRunner.load_env_local()


def env_override(m):
    return IpIntelligenceTestRunner.env_override(m)


def entity_data(v):
    return IpIntelligenceTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return IpIntelligenceTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return IpIntelligenceTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return IpIntelligenceTestRunner.load_test_control()


def live_client_options():
    return IpIntelligenceTestRunner.live_client_options()


def live_delay_ms():
    return IpIntelligenceTestRunner.live_delay_ms()
