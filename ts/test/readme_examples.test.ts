...
//     (\`new IpIntelligenceSDK(...)\` / \`IpIntelligenceSDK.test(...)\` become
//     \`IpIntelligenceSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'IpIntelligenceSDK'
...
const TEST_SEED = {"entity":{"api":{"test01":{"id":"test01"}},"usage":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new IpIntelligenceSDK(...)\` and any
// \`IpIntelligenceSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+IpIntelligenceSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/IpIntelligenceSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
