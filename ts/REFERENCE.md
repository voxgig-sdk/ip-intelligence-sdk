# IpIntelligence TypeScript SDK Reference

Complete API reference for the IpIntelligence TypeScript SDK.
...
```ts
new IpIntelligenceSDK(options?: object)
```
...
#### `IpIntelligenceSDK.test(testopts?, sdkopts?)`
...
**Returns:** `IpIntelligenceSDK` instance in test mode.
...
Alias for `IpIntelligenceSDK.test()`.
...
| `metadata` | `Record<string, any>` | No | Additional contextual information about the IP, structure varies based on classifications |
...
Return the parent `IpIntelligenceSDK` instance.
