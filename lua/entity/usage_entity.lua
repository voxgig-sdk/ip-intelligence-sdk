...
local UsageEntity = {}
UsageEntity.__index = UsageEntity


function UsageEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, UsageEntity)
  self._name = "usage"
  self._client = client
...
function UsageEntity:get_name()
  return self._name
end


function UsageEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return UsageEntity.new(self._client, opts)
end
...
function UsageEntity:mark_deleted()
  self._deleted = true
end


function UsageEntity:deleted()
  return true == self._deleted
end


function UsageEntity:data_set(args)
  if args ~= nil then
...
function UsageEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function UsageEntity:match_set(args)
  if args ~= nil then
...
function UsageEntity:match_get()
...
function UsageEntity:stream(action, args, callopts)
...
---@param reqmatch UsageLoadMatch
---@param ctrl? table
---@return Usage
---@return string? err
function UsageEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function UsageEntity:_run_op(ctx, post_done)
  local utility = self._utility
