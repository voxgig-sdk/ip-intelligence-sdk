...
[MCP](https://modelcontextprotocol.io) server exposing the IpIntelligence SDK as
two agent tools — `ip-intelligence_list` and `ip-intelligence_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/ip-intelligence-mcp)
make build

# 2. Provide credentials via the environment
export IP_INTELLIGENCE_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user ip-intelligence \
  -- /absolute/path/to/ip-intelligence-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./ip-intelligence-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// ip-intelligence_list: first page of records
{ "entity": "api" }
{ "entity": "api", "query": { } }

// ip-intelligence_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/ip-intelligence-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user ip-intelligence \
     -- "$PWD"/dist/*/ip-intelligence-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `ip-intelligence_list` and `ip-intelligence_load` tools now appear
   in new sessions. Ask the agent to *"list api using ip-intelligence"*
   and it calls `ip-intelligence_list` with `{"entity":"api"}`.
...
```sh
export IP_INTELLIGENCE_APIKEY=sk_live_xxx            # API key
export IP_INTELLIGENCE_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `ip-intelligence_list` tool
...
### Call the `ip-intelligence_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `ip-intelligence_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `ip-intelligence_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `IP_INTELLIGENCE_APIKEY` | API key sent with every request. |
| `IP_INTELLIGENCE_BASE` | Optional override of the API base URL. |
...
```sh
./ip-intelligence-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"ip-intelligence_load","arguments":{"entity":"api","query":{"id":1}}}}'
```
