...
boru-driven command-line client **and** interactive REPL for the IpIntelligence
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/ip-intelligence-cli)
make build

# 2. See usage (words, entities, env vars)
./ip-intelligence-cli --help

# 3. Provide credentials once, via the environment
export IP_INTELLIGENCE_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./ip-intelligence-cli load 1 api            # {id:1} shorthand
./ip-intelligence-cli load '{id:1}' api       # explicit match map

# 5. Override the API base URL for a single call
IP_INTELLIGENCE_BASE=https://api.example.com ./ip-intelligence-cli load 1 api

# 6. No arguments -> interactive REPL
./ip-intelligence-cli
ip-intelligence> load 1 api
ip-intelligence> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/ip-intelligence-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/ip-intelligence-cli load 1 api
   ```
...
```sh
./ip-intelligence-cli load 1 api          # scalar shorthand for {id:1}
./ip-intelligence-cli load '{id:1}' api     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `ip-intelligence>`). Each line is
evaluated as its own boru expression:

```text
$ ./ip-intelligence-cli
ip-intelligence> load 1 api
ip-intelligence> /help
ip-intelligence> /quit
```
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/ip-intelligence-cli`. |
...
(`./ip-intelligence-cli <expr>`) and interactively (the REPL), and expressions compose the
