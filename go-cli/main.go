// Command IpIntelligence-cli is a CLI + REPL for the IpIntelligence SDK,
...
const prompt = "ip-intelligence"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "api usage"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      IP_INTELLIGENCE_APIKEY (api key), IP_INTELLIGENCE_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: IP_INTELLIGENCE_APIKEY carries the API key and
	// IP_INTELLIGENCE_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("IP_INTELLIGENCE_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("IP_INTELLIGENCE_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewIpIntelligenceSDK(opts)
