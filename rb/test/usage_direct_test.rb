...
class UsageDirectTest < Minitest::Test
  def test_direct_load_usage
    setup = usage_direct_setup({ "id" => "direct01" })
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-load-usage", setup[:live] ? "live" : "unit")
    if _should_skip
...
def usage_direct_setup(mockres)
