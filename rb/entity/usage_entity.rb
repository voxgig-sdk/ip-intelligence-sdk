...
class UsageEntity
  ...
  def make
    opts = @_entopts.dup
    UsageEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = IpIntelligenceHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Usage, Hash] the current Usage data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = IpIntelligenceHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Usage fields)
  def match_get
  ...
  # Load a single Usage.
  #
  # @param reqmatch [UsageLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Usage.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Usage, Hash] the loaded Usage; raises IpIntelligenceError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = IpIntelligenceHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
