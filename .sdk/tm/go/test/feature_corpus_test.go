...
//
// NewIpIntelligenceSDK, not TestSDK: the `test` feature is transport: 'base' and
// REPLACES the transport, so a client in test mode would shadow the script.
func fcClient(kase map[string]any) *sdk.IpIntelligenceSDK {
	res, _ := kase["res"].([]any)
	...
	return sdk.NewIpIntelligenceSDK(opts)
}
...
func fcCandidates(client *sdk.IpIntelligenceSDK) []fcOp {
	out := []fcOp{}
...
func fcInvoke(client *sdk.IpIntelligenceSDK, op fcOp, ctrl map[string]any) error {
	cv := reflect.ValueOf(client)
	ct := cv.Type()
	mapType := reflect.TypeOf(map[string]any{})
...
// new go here.
func fcRecord(client *sdk.IpIntelligenceSDK, name string) any {
			...
			for _, raw := range cases {
				...
				for _, rawstep := range steps {
					...
					if code, ok := wanterr.(string); ok {
						...
						// the word.
						got := ""
						var sdkerr *sdk.IpIntelligenceError
