...
module IpIntelligenceUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = IpIntelligenceResponse.new({})
    ...
    if fetch_err
      ...
      response = IpIntelligenceResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = IpIntelligenceResponse.new(fetched)
    else
