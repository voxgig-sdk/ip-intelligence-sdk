...
class IpIntelligenceMakeError
{
    public static function call(?IpIntelligenceContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new IpIntelligenceContext([], null);
        }
        $op = $ctx->op ?? new IpIntelligenceOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new IpIntelligenceResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof IpIntelligenceError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new IpIntelligenceError('', $msg, $ctx);
        $sdk_err->result = ($ctx->utility->clean)($ctx, $result);
        $sdk_err->spec = ($ctx->utility->clean)($ctx, $spec);
        if ($err instanceof IpIntelligenceError) {
