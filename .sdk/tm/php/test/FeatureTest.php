...
// IpIntelligence SDK feature test
//
...
class FtCtrl extends IpIntelligenceControl
{
...
class FtClient
{
    ...
    private IpIntelligenceUtility $utility;

    public function __construct(IpIntelligenceUtility $utility, array $options)
    {
    ...
    public function get_utility(): IpIntelligenceUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (IpIntelligenceContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public IpIntelligenceContext $rootctx;
    ...
    public function __construct(
        ...
        $utility = new IpIntelligenceUtility();
        $this->utility = $utility;

        $utility->fetcher = $server ?? function (IpIntelligenceContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = IpIntelligenceConfig::make_feature($name);
    ...
    private static function buildUrl(IpIntelligenceSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof IpIntelligenceError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new IpIntelligenceResponse($fetched);
        }
    ...
    private function fail(IpIntelligenceContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (IpIntelligenceContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
